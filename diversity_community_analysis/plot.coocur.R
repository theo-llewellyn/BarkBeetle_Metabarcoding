# new function at 0.025 significance
plot.cooccur <-
  function(x, ...){
    ...
    comat <- ifelse(comat_pos>=0.025,0,1) + ifelse(comat_neg>=0.025,0,-1)
    ...
    if(randsummary == FALSE){  
      ...
      p <- p + geom_text(data=dfids,aes(label=X1),hjust=1,vjust=0,angle = -22.5, size = 2)#, color="dark gray")
